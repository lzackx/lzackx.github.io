...
&emsp;&emsp;Flutter与原生的差异导致开发Flutter的时候，需要选择另外一个代码编辑器开发，例如Android Studio，但是Android又不能很好地让Xcode协助调试，于是在调试Flutter时，基本步骤如下（详细图解闲时补充）：

* Xcode启动App
* 打开由Flutter开发的页面
* 在Android Stuido中基于Flutter打开页面弹出的VM监听入口打印的地址进行`flutter attach`
...
&emsp;&emsp;仔细看时序图，在16ms上有一条线，这就是断定渲染当前帧是否满足60FPS的一个标准。

&emsp;&emsp;另外，出了上面提到的2个性能监控能力外，在顶部的工具栏还有一些能在App上展现出来的辅助工具，可以直接从页面上看到性能监控的情况。
